...
// Banner pode ser facilmente trocado depois
const BANNER_URL = "/images/Foto- Jake_Lyell_3.webp";
...
export default function DoacaoMensalForm() {
  const { user, isAuthenticated } = useAuth();
  const { addDonation } = useLocalDonations();
  const { updateDonationFields, updateUserFields, updatePaymentFields } = useDonation();
  const { 
    processPayment,
    modalStage,
    modalMessage,
  ...
  const handlePaymentMethodSelection = (method: 'credito' | 'debito') => {
    console.log('🔍 [DoacaoMensal] Método selecionado:', method, 'Autenticado:', isAuthenticated);
    
    if (method === 'debito' && !isAuthenticated) {
      console.log('🚨 [DoacaoMensal] Débito selecionado sem login - abrindo modal local');
  ...
  useEffect(() => {
    if (isAuthenticated && user) {
      console.log('🔍 [DoacaoMensalForm] Carregando dados do usuário logado:', user);
      ...
      // (nome no cartão pode ser diferente do nome no cadastro)
      
      console.log('✅ [DoacaoMensalForm] Dados básicos preenchidos automaticamente');
    }
  }, [isAuthenticated, user]);

  // Mensagem de impacto dinâmica
  const getImpactMessage = (v: number) => {
    if (v >= 100) return `Com R$ ${v} você garante material escolar para 5 crianças.`;
    if (v >= 80) return `Com R$ ${v} você garante alimentação para 4 crianças.`;
    if (v >= 50) return `Com R$ ${v} você garante alimentação para 2 crianças.`;
    if (v >= 20) return `Com R$ ${v} você contribui para o transporte escolar de uma criança.`;
    return "Sua doação faz a diferença!";
  };
  ...
  function validarEtapa3() {
    console.log('🔍 [DoacaoMensalForm] validarEtapa3 iniciado');
    console.log('🔍 [DoacaoMensalForm] Estado atual:', { metodo, pagamento });
    ...
    setErrosPag(e);
    console.log('🔍 [DoacaoMensalForm] validarEtapa3 resultado:', { validCard, erros: e });
    return validCard;
  }
  ...
  const processarDoacao = async () => {
    ...
    try {
      ...
      updateDonationFields({
        ...
        paymentMethod: metodo === 'credito' ? 'credit_card' : 'bank_transfer',
        pay_duo_date: metodo === 'debito' ? pagamento.diaDebito : '05'
      });

      // ✅ CORREÇÃO: Atualizar dados pessoais no contexto se não estiver logado
      if (!isAuthenticated) {
        console.log('🔍 [DoacaoMensalForm] Atualizando dados pessoais no contexto:', dados);
        updateUserFields({
          name: dados.nome,
          email: dados.email,
          document: dados.cpf,
        ...
        // ✅ CORREÇÃO: Atualizar dados de pagamento no contexto
        if (metodo === 'credito') {
          console.log('🔍 [DoacaoMensalForm] Atualizando dados de cartão de crédito:', pagamento);
          const validadeParts = pagamento.validade.split('/'); // MM/YYYY
          const mes = validadeParts[0] || '';
          const ano = validadeParts[1] || '';
          ...
          console.log('🔍 [DoacaoMensalForm] Atualizando dados de débito automático:', pagamento);
          updatePaymentFields({
            paymentMethod: 'bank_transfer',
            bankCode: pagamento.banco,
            agency: pagamento.agencia,
            account: pagamento.conta,
            accountType: 'corrente', // Valor padrão pois não existe tipoConta no estado
            debit: {
              pay_name: dados.nome,
              pay_doc: dados.cpf,
      ...
      // Process payment using the new system passando dados diretamente
  ...
  // Responsividade: container com padding e espaçamento
  return (
    <div className="w-full max-w-2xl mx-auto bg-white rounded-2xl shadow-lg mt-12 mb-16 flex flex-col overflow-hidden">
      ...
      <div className="w-full aspect-[3/1] md:aspect-[3/1.2] bg-gray-200">
        <img src={BANNER_URL} alt="Crianças sorrindo" className="object-cover w-full h-full" style={{ display: 'block' }} />
      </div>
      <div className="px-2 sm:px-4 md:px-12 py-8 flex-1">
        <h1 className="text-2xl sm:text-3xl font-bold text-childfund-green text-center mb-2">Guardião da Infância</h1>
        <p className="text-base sm:text-lg text-center text-gray-700 mb-4 font-medium">Seja um Guardião da Infância e transforme a vida de crianças e adolescentes com apoio mensal contínuo.</p>
        {/* Steps */}
      ...
      <div className="flex flex-col gap-8">
        <div>
            ...
            <input
              ...
              <div className="bg-green-50 border border-green-200 rounded-lg p-4 text-green-800 flex flex-col sm:flex-row items-start sm:items-center gap-2">
                ...
                <span className="ml-0 sm:ml-2 text-sm">{getImpactMessage(valorSelecionado)}</span>
              </div>
            </div>
        ...
        {step === 3 && (
          ...
          e.preventDefault(); 
          console.log('🔍 [DoacaoMensalForm] Form submit da etapa 3');
          const isValid = validarEtapa3();
          console.log('🔍 [DoacaoMensalForm] Validação passou:', isValid);
          if (isValid) { 
            console.log('🔍 [DoacaoMensalForm] Chamando processarDoacao');
            await processarDoacao(); 
          } else {
            console.log('❌ [DoacaoMensalForm] Validação falhou - não processando doação');
          }
        }}>
            <div>
            ...
            <div className="flex gap-2 justify-between mt-4">
              <Button type="button" variant="outline" onClick={() => setStep(2)} className="w-1/2" disabled={isProcessing}>
                Voltar
              </Button>
              <Button 
                type="submit" 
                className="w-1/2 bg-childfund-green" 
                disabled={!acceptedPrivacyPolicy || isProcessing}
              >
                {isProcessing ? "Processando..." : "Finalizar Doação"}
              </Button>
            </div>
          </form>
        )}
        ...
        {/* Próximas etapas virão depois da sua aprovação */}
      </div>
      
      {/* Tracking RD Station */}
      <RDEventTracker
        type="Iniciou Doação Recorrente"
        email={dados.email}
        name={dados.nome}
        phone={dados.telefone}
        state={dados.estado}
